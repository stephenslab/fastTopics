...
\begin{equation}
x_i \sim \mathrm{Binom}(n_i, \pi_i),
\label{eq:binomial}
\end{equation}
where $x_i$ is the expression level of the gene in sample $i$, $n_i$
...
differential gene expression: more specifically, $\log_2(f_1/f_0)$
quantifies the {\em relative} log-fold change. Consider the following
alternative gene expression model:
\begin{align}
\end{align}
where ... [Give (relative) log-fold change statistic.] This model is
in fact is equivalent to the binomial model \eqref{eq:binomial model},
and therefore estimating $f_0, f_1$ in the binomial model provides an
