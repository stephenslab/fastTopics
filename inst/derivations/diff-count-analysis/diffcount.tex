...
expression level between two conditions. It is often preferrable to
measure {\em relative} change, say, change in gene expression relative
to the total expression in each sample. As we will see, a topic
modeling perspective provides a natural way to analyze both absolute
or relative change in gene expression.

Topic modeling brings two new twists to analysis of differential gene
expression:
\begin{enumerate}

\item In conventional differential expression analysis, each sample is
  assigned to a single condition; in topic modeling, the topic
  assignments are proportional.

\item In conventional differential expression analysis, all genes
  expression measurements in a sample are assumed to be from the same
  condition; in topic modeling, the topic assignments are made at the
  level of genes, not samples.

\end{enumerate}
We will need to bear in mind these two points in developing the models
and methods.

\subsection{The binomial model}

We begin with a simple binomial model of gene expression given
the proportional assignments to a topic,
\begin{equation}
x_i \sim \mathrm{Binom}(s_i, \pi_i),
\label{eq:binomial}
\end{equation}
where $x_i$ is the expression level of the gene in sample $i$, $s_i$
is the total expression in sample $i$, and $\mathrm{Binom}(n, \theta)$
denotes the binomial distribution with $n$ trials and success
probability $\theta$. In this model, the binomial probabilities are
defined as
\begin{equation}
\pi_i = (1 - q_i) f_0 + q_i f_i,
\label{eq:binomial-prob}
\end{equation}
where $q_i \in [0,1]$ is the known proportion of sample $i$ attributed
to the topic, and $f_0, f_1 \in [0, 1]$ are two unknowns to be
estimated.

In econometrics, this model is sometimes called a {\em linear
  probability model} \cite{stock-watson}. This viewpoint is more
apparent by rewriting the binomial probabilities as
\begin{equation}
\pi_i = \beta_0 + q_i \beta,
\end{equation}
where $\beta_0 = f_0$ and $\beta = f_1 - f_0 \in [-1, +1]$. This is a
simple regression model for the binomial probability, in which the
binomial probability increases linearly with topic proportion $q_i$,
with the slope given by $\beta$. 

Statistical inference with this simple binomial model implements
differential gene expression---in particular, $\log_2(f_1/f_0)$
quantifies the {\em relative log-fold change.} To show this, consider
...
\item \begin{enumerate}
  ...
  \mathrm{Binom}(1,f_1) & \mbox{if $z_{it} = 1$} \\
  \mathrm{Binom}(1,f_0) & \mbox{otherwise.}
  \end{array}\right.$

  \end{enumerate}

  \item $x_i \leftarrow w_{i1} + \cdots + w_{is_i}$.

\end{enumerate}
\end{itemize}
\vspace{1em}
In this statistical process, $f_1 = p(w_{it = 1} \,|\, z_{it} = 1)$ is
the conditional probability that the gene is expressed given
membership to the topic, and $f_0 = p(w_{it = 1} \,|\, z_{it} = 0)$ is
the probability that the gene is expressed when not belonging to the
...
\label{eq:lfc-binom}
\end{equation}
is the relative log-fold change statistic given the proportional topic
assignments $q_1, \ldots, q_n$. The binomial model \eqref{eq:binomial}
can in fact be derived from this statistical process, so estimating
$f_0, f_1$ for the binomial model \eqref{eq:binomial} provides an
estimate of the (relative) log-fold change \eqref{eq:lfc-binom}.
