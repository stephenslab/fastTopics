...
plot(dat$F[,1],dat$F[,2],pch = 4,cex = 0.5,log = "xy")
abline(a = 0,b = 1,col = "magenta",lty = "dotted")

# Fit a Poisson model (approximating a binomial model) to each
# combination of gene j and topic k.
s   <- rowSums(X)
fit <- fit_univar_poisson_models(X,L,s,method = "em-rcpp")

# For the selected topic, compare the f1 estimates against the
# probabilities used to simulate the data.
i <- 1
plot(dat$F[,i],fit$F1[,i],pch = 4,cex = 0.5,log = "xy",
...
plot(rowMeans(dat$F[,-i]),fit$F0[,i],pch = 4,cex = 0.5,log = "xy",
     xlab = "true f0 (approx)",ylab = "estimated f0")
abline(a = 0,b = 1,col = "magenta",lty = "dotted")

# Compute the log-fold change and Z-score for each gene j and topic k.
out <- compute_univar_poisson_zscores(X,L,fit$F0,fit$F1,s)
