...
but this results in quite different estimates of L and F:
...
So this suggests that using the multinomial topic model is the wrong
thing to do if the data are indeed binomial.

Using this same Poisson approximation to the binomial, we can perform
also a grade-of-membership (GoM) differential expression analysis,
which is based on a Poisson model of the binary data:

```{r de-analysis, results="hide"}
...
Again, to underscore that using the multinomial topic model is not a
good idea when the data are binomial, a GoM DE analysis based on the
multinomial topic model does not as effectively distinguish the large
changes from the zero or near-zero changes:
