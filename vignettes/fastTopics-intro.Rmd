...
knitr::opts_chunk$set(collapse = TRUE,results = "hold",comment = "#",
                      fig.align = "center",fig.height = 4.5,fig.width = 6)
```
...
are very simple and fast, so we can run a lot of theem in a
relatively short amount of time. The EM algorithm is recovered by
turning off the SQP updates in `altsqp`.

```{r run-betanmf}
timing1 <- system.time(
  fit1 <- altsqp(droplet,list(L = L,F = F),numiter = 250,
                 control = list(numsqp = 0,nc = 28)))
```

Now try the SQP method from the fastTopics package. (You may want to
adjust the `nc` control parameter to match the number of available
cores on your computer.)

```{r run-altsqp}
timing2 <- system.time(
  fit2 <- altsqp(droplet,list(L = L,F = F),numiter = 40,
                 control = list(extrapolate = 20,nc = 28)))
```
...
calling `poisson2multinom`, the `altsqp` can also be used to recover
maximum-likelihood estimates of the topic probabilities ("loadings")
and gene probabilities ("factors") for a topic model of gene
expression.
