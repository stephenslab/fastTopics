...
RNA-seq data using a topic model. Here we revisit the same data set,
giving more detailed guidance and discussing some complications that may
arise.
...
interface hides most of the complexities of model fitting.
Nonetheless, it is prudent to check that the model fitting has
converged to a solution---that is, a local maximum of the likelihood
surface. This can be done conveniently using a "progress plot":
...
topic model, so the plot shows the progress of the NMF model fit.)
Judging by this plot, which shows the change in the log-likelihood
over time, the iterates get close to a maxium-likelihood solution
