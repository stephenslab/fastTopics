...
```{r loglik-3, fig.height=2, fig.width=4.5}
subpop_colors <- c("dodgerblue","forestgreen","darkmagenta","skyblue","gold")
pdat <- data.frame(loglik = loglik,subpop = pbmc_4k$samples$subpop)
...
dfa_merged <- diff_count_analysis(fit_merged,counts)
volcano_plot(dfa_merged,k = "k5+k6",label_above_quantile = 0.998,
...
```{r volcano-plotly}
volcano_plotly(dfa_merged,k = 5,file = "volcano_plot_t_cells.html",
               labels = genes$symbol)
```
...
volcano_plot(dfc_out,k = "CD8+",label_above_quantile = 0.995,
...
Finally, to isolate the differences in expression between topics 5 and
6 capturing heterogeneity within T cells, we perform a differential
expression analysis on the T cells only:
...
volcano_plot(diff_count_t,k = 5,label_above_quantile = 0.995,
             labels = genes$symbol,max.overlaps = 20)
```

Judging by top differentially expressed genes such as *CD4*, topic 5
likely corresponds to CD4+/CD8+ lineage differentiation in T cells.
