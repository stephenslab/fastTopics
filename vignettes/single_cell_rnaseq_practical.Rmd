...
Interpreting the topics without cell labels
-------------------------------------------
...
Without the cell labels, the Structure plot is still able to highlight
some of the key substructures:
...
factors (in particular, factor 6) among NK and T cells.
...
While the Structure plot worked fairly well for this example to depict
the cell types and T cell diversity captured by the topics, in more
complex data sets finer scale structure or rare cell types may not
show up in the Structure plot. Therefore, for more complex data sets,
additional strategies will be needed.

One strategy that we have found to be generally useful is to identify
clusters in the mixture proportions matrix $L$. Here, we illustrate
this idea using the simplest clustering method, *k*-means. We have had
better success with applying $k$-means to the principal components
(PCs) of the mixture proportions, which can be easily computed using
`prcomp`.
...
membership to each of the 6 clusters:
...
refinement will often be needed to get the "right" clustering,
regardless of which clustering method is used, *k*-means or otherwise.
...
until downstream analyses have been conducted.
...
We emphasize that there is no one clustering technique that will work
well for all data sets---in particular, we do not expect *k*-means to
work as well in complex data sets. This is only meant to be an
illustration of the *idea* of identifying clusters from the mixture
proportions, and you may find that some ingenuity will be needed to
adapt these ideas to your data set.
