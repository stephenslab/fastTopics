...
vs. >10 in the topic. This suggests that the topic model better
isolates expression patterns specific to B cells.

There is no single topic for T cells.

*Add text here.*

```{r volcano-plot-cd4, fig.height=3.5, fig.width=4}
i <- which(clusters == "T")
fit_t <- select_loadings(fit_multinom,loadings = i)
diff_count_t <- diff_count_analysis(fit_t,counts[i,])
p9 <- volcano_plot(diff_count_t,k = 5,label_above_quantile = 0.995,
