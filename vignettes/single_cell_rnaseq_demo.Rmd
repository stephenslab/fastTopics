...
In addition to the Matrix and fastTopics packages, we also use ggplot2
and cowplot to visualize the results of the topic modeling analysis.
...
easly recovered by a simple reparameterization:

```{r poisson2multinom}
fit_multinom <- poisson2multinom(fit)
```

Structure plot---a first attempt
--------------------------------
...
```{r structure-plot, fig.width=5.75, fig.height=1.5, message=FALSE, results="hide"}
