...
In this last part of the analysis, we perform an analysis of
differential expression, and use the results of this analysis to
annotate clusters and factors ("topics"). We begin by analyzing
differential expression in the 6 clusters.

```{r diff-count-analysis-clusters}
fit_clusters <- init_poisson_nmf_from_clustering(counts,clusters)
...
```{r volcano-plot-bcell-cluster, fig.height=3, fig.width=3}
...
```{r volcano-plot-tcell-cluster, fig.height=3, fig.width=3}
...
```{r volcano-plot-nk-cluster, fig.height=3, fig.width=3}
p8 <- volcano_plot(diff_count_clusters,k = "NK",label_above_quantile = 0.998,
                   labels = genes$symbol)
print(p8)
```
...
```{r volcano-plot-bcell-topic, fig.height=3, fig.width=3}
p9 <- volcano_plot(diff_count_topics,k = 4,label_above_quantile = 0.998,
                   labels = genes$symbol)
print(p9)
```
...
```{r scatterplot-1, fig.height=3, fig.width=3}
j <- which(abs(diff_count_clusters$Z[,"B"]) > 4 &
           abs(diff_count_topics$Z[,4]) > 4)
pdat <- data.frame(cluster = diff_count_clusters$beta[j,"B"],
                   topic = diff_count_topics$beta[j,4])
p10 <- ggplot(pdat,aes(x = cluster,y = topic)) +
...
Explain that there is no single topic for T cells.

*Add text here.*

```{r volcano-plot-nk-topic, fig.height=3, fig.width=3}
p11 <- volcano_plot(diff_count_topics,k = 1,label_above_quantile = 0.998,
                    labels = genes$symbol)
print(p11)
```
...
```{r volcano-plot-cd4, fig.height=3, fig.width=3}
i <- which(clusters == "T")
fit_t <- select(fit_multinom,loadings = i)
diff_count_t <- diff_count_analysis(fit_t,counts[i,])
p12 <- volcano_plot(diff_count_t,k = 5,label_above_quantile = 0.99,
                    labels = genes$symbol)
print(p12)
```

TO DO: Create an interactive volcano plot.
