...
fit the multinomial topic model. Although fast, this can still take
several minutes to run, or more, so we have saved a "pre-fitted" model:
...
algorithm initially makes rapid progress toward a solution. However,
this convergence slows considerably as the iterates approach the
solution, and close to a solution we have found that other algorithms
work much better. Therefore, we have found that a two-stage model
fitting approach works well: first perform EM updates, then refine the
fit using extrapolated SCD updates. For larger data sets, do not be
surprised if more than 200 updates are needed to obtain a good fit.
...
vIn some areas of the the PCA plot, there are many---sometimes
