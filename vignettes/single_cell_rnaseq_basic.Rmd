...
not normalize or transform the counts.* The reason we don't need to do
this is that "raw" counts are modeled directly (in particular, the
counts are modeled using a multinomial distribution).
...
the UMI counts are zero. These genes should be removed prior to the
