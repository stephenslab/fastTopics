...
expression, is shown on the *y*-axis:
...
Another useful feature of the topic model is that we can compute a
multinomial likelihood for each cell to evaluate how well the topic
model "fits" the observed expression in the cell:

```{r loglik-1}
loglik_topics <- loglik_multinom_topic_model(counts,fit)
```

This could be useful for comparing topic models (say, at different
$K$), or for identifying "outlier" cells with expression that is
poorly captured by the topic model:

```{r loglik-2, fig.height=2, fig.width=3}
p4 <- ggplot(data.frame(x = loglik_topics),aes(x)) +
...
In this histogram, there is a small number of cells with 
