...
Consider a standard differential expression (DE) analysis using
cell-type labels.  This analysis centers on calculation of the
*log-fold change* (LFC); for a given cell type, the LFC is
defined as the (base-2) log-ratio of two expression levels, the
estimated expression in cells belonging to the group over the
expression in cells not belonging to the group.

'fastTopics' extends the standard DE analysis to allow for *partial
membership* to multiple groups. This grade of membership differential
expression (GoM DE) analysis is implemented by the function
`de_analysis`:
