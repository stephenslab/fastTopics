...
As in the standard analysis, it outputs log-fold change estimates:
...
*CD79A*. In fact, *CD79A* is nearly uniquely expressed in topic 4.
...
and *CD79B*, appear near the top-right corner of the volcano plot. 

(Note that the x-axis in the volcano plot ranges between -10 and 10; by
default, log-fold change estimates outside this range are projected
onto this range. You can use the `betamax` argument to change how the
log-fold change estimates are shown in the volcano plot.)
...
See [Part 2][vignette-part-2] for more examples of differential
expression analysis using a topic model.
