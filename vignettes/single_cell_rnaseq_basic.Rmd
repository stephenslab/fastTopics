...
analysis of single-cell RNA-seq data using a topic model. See
[Part 2][vignette-part-2] for additional guidance.
...
such as the tissue the cells were sampled from. (See
[Part 2][vignette-part-2] for how to interpret the topics when cell
labels are unavailable.) In the PBMC 4k data, the cells were labeled
...
dfc_out <- diff_count_clusters(counts,samples$subpop)
...
volcano_plot(dfa_out,k = 4,label_above_quantile = 0.998,
...
volcano_plot(dfa_out,k = 1,label_above_quantile = 0.998,
             labels = genes$symbol,max.overlaps = 15)
```

Interpreting the topics that capture variation within the T cells is a
little more complicated, so we defer this investigation to
[Part 2][vignette-part-2].
