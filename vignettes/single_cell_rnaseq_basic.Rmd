...
the biological relevance of the topics. We turn to this question next.

Interpreting the topics using available cell labels
---------------------------------------------------

In some situations, there may be additional information about the
cells, such as the tissue the cells were sampled from. Here, using
fluorescence-activated cell sorting (FACS), the cells are labeled
according to 5 cell types, B cells, CD14+ monocytes, CD34+ cells,
natural killer (NK) cells, and T helper cells:

```{r summary-subpop}
summary(pbmc_4k$samples$subpop)
```

To help us understand the relationship between the cell types and the
topics, we create a Structure plot summarizing the mixture proportions
in each of the 5 cell types:
...
topic corresponds to a bar of a different colour. Since the mixture
proportions for each cell sum to 1, the total height of the bars is
the same for each cell, making it easier to compare across cells.
Patterns emerge when the cells are arranged so that cells with similar
mixture proportions are nearby each other along the horizontal
axis. The arranging of the cells within each cell type is automated in
`structure_plot` using the *t*-SNE method.
...
6). As we will see next, we can analyze differences in gene expression
among the topics to help us understand what aspects of T cell
diversity are captured by these three topics.
