...
Poisson NMF model, and nex we will show that the likelihood does not
change with this reparameterization. Specifically, we have the
following result: $$ \mathrm{Multinom}(x_i; t_i, \pi_i) \times
\mathrm{Poisson}(t_i; s_i) = \prod_{j=1}^m \mathrm{Poisson}(x_{ij};
\lambda_{ij}), $$ in which the $s_i$'s are the "size factors". We can
check that this result holds in the toy data:
...
We can also easily go in the reverse direction, from Poisson NMF fit
to multinomial topic model fit. We give an example (we remove the size
factor estimates returned by `poisson2multinom` to show that they are
not needed):
...
The likelihoods for the original Poisson NMF fit, and the one
recovered from the topic model, are exactly the same (note the factors
and loadings are not necessarily the same---they will only be the
same up to a rescaling of the columns).

Summary
-------

This approach combines the advantages of Poisson NMF and topic modeling.
