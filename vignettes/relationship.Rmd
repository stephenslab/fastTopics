...
parameters $f_{jk} \geq 0, l_{ik} \geq 0$ to be fitted the data:
$$
\lambda_{ij} = \sum_{k=1}^K l_{ik} f_{jk}.
$$
Here, $K$ is a user-specified tuning parameter. It is the "rank" of the
...
$$
Note the similarity to the Poisson rates $\lambda_{ij}$ above.  There
is, however, a key difference: the parameters $l_{ik}^{\ast} \geq 0,
f_{jk}^{\ast} \geq 0$ are additionally constrained so that each row of
the loadings matrix $L^{\ast}$ and each column of factors matrix
$F^{\ast}$ sums to 1:
$$
\sum_{k=1}^K l_{ik}^{\ast} = 1, \qquad
\sum_{j=1}^m f_{jk}^{\ast} = 1.
$$
With the addition of these constraints, the $\pi_{ij}$'s are
guaranteed to be probabilities, and the factors and loadings have a
simple interpretation: $l_{ik}^{\ast}$ is the proportion that
topic $k$ contributes to the counts in sample $i$; $f_{jk}$ is the
probability of token $j$ occuring in topic $k$.
...
Indeed, therows of $L$ and columns of $F$ sum to 1:
...
this reparameterization. Specifically, we have the following result:
$$
\mathrm{Multinom}(x_i; t_i, \pi_i) \times \mathrm{Poisson}(t_i; s_i) =
\prod_{j=1}^m \mathrm{Poisson}(x_{ij}; \lambda_{ij}),
$$
in which the $s_i$'s are the mean estimates of the document sizes
 $t_i$. We can check that this result holds in the toy data:
...
We can also go in the reverse direction and recover a Poisson NMF fit
from a multinomial topic model fit, even when the sample sizes $s_i$
have not been estimated. We give an example to illustrate:

```{r multinom2poisson, fig.height=3, fig.width=3.25}
fit.multinom["s"] <- NULL
...
This approach combines the benefits of Poisson NMF and topic modeling.
