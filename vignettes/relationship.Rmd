...
Also, we can easily go in the reverse direction, from Poisson NMF fit
to multinomial topic model fit. Here is an example, again using the
toy data (we remove the size factor estimates returned by
`poisson2multinom` to show that they are not needed):
...
same---they will the same up to a rescaling of the columns).
